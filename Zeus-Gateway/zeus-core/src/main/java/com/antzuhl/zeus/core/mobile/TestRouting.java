...
public class TestRouting extends ZeusFilter {
	...
	private static final DynamicStringProperty ROUTING_TABLE_STRING_PROPERTY = DynamicPropertyFactory.getInstance()
			.getStringProperty("zeus.routing_table_string", null);
	...
	public boolean shouldFilter() {
		...
		return ctx.sendZeusResponse();
	}

	// sample url
	// http://api.antzuhl.com/api/hello
	@Override
		...
		if (StringUtils.isNotEmpty(serviceName)) {
			...
			if (StringUtils.isNotEmpty(urlString)) {
				...
				try {
					...
					throw new ZeusException(e, "Malformed URL exception", 500, "Malformed URL exception");
				}
			}
		}

		if (ctx.getRouteUrl() == null) {
			throw new ZeusException("No route found", 404, "No route found");
		}
