...
public class MobileExecuteRoute extends ZeusFilter {
	...
	public Object run() throws ZeusException {
		...
		try {
			...
			Collection<Header> headers = buildZeusRequestHeaders(request);
			...
			throw new ZeusException(errorMsg,500, String.format("TargetUrl: %s\nCause: %s",targetUrl,errorMsg));
		} finally {
			tran.complete();
		}
	...
	public void setResponse(HttpResponse response) throws Exception, IOException {
		...
		for (Header header : response.getAllHeaders()) {
			if (header.getName().equalsIgnoreCase(ZeusHeaders.CONTENT_LENGTH)) {
				ctx.setOriginContentLength(header.getValue());
			}
			if (isValidZeusResponseHeader(header.getName())) {
				 ctx.addZeusResponseHeader(header.getName(), header.getValue());
			}
			if (header.getName().equalsIgnoreCase(ZeusHeaders.CONTENT_ENCODING)) {
	...
	public Collection<Header> buildZeusRequestHeaders(HttpServletRequest request) {
		...
		for (Entry<String, String> entry : zeusRequestHeaders.entrySet()) {
	...
	public HttpResponse forward(HttpClient httpclient, RequestConfig requestConfig, String verb, String url,
		...
		if (isolationStrategy == null) {
			isolationStrategy = DynamicPropertyFactory.getInstance().getStringProperty("zeus.isolation.strategy.global", "SEMAPHORE").get();
		}

		long start = System.currentTimeMillis();
		try {
			if ("THREAD".equalsIgnoreCase(isolationStrategy)) {
				return new ZeusRequestCommandForThreadIsolation(httpclient, httpUriRequest, groupName, routeName).execute();
			} else {
				return new ZeusRequestCommandForSemaphoreIsolation(httpclient, httpUriRequest, groupName, routeName).execute();
			}
		} finally {
	...
	public boolean isValidZeusResponseHeader(String name) {
	...
	private RequestConfig buildRequestConfig(String routName, String groupName) {
		...
		if (connectTimeout == 0) {
			connectTimeout = DynamicPropertyFactory.getInstance().getIntProperty("zeus.connect.timeout.global", 2000)
					.get();
		}
		...
		if (socketTimeout == 0) {
			socketTimeout = DynamicPropertyFactory.getInstance().getIntProperty("zeus.socket.timeout.global", 1000)
					.get();
		}
		...
		if (requestConnectionTimeout == 0) {
			requestConnectionTimeout = DynamicPropertyFactory.getInstance()
					.getIntProperty("zeus.request.connection.timeout.global", 10).get();
		}
