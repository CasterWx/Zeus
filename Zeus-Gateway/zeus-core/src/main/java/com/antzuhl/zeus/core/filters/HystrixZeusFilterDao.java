...
public class HystrixZeusFilterDao implements IZeusFilterDao {
	
	private IZeusFilterDao delegation;

	public HystrixZeusFilterDao(IZeusFilterDao delegation){
	...
	public List<FilterInfo> getZeusFilters(String filterId) throws Exception {
        return new GetZeusFiltersCommand(filterId).execute();
	}
    ...
    public abstract class AbstractCommand<R> extends HystrixCommand<R> {
        protected AbstractCommand() {
            super(Setter.withGroupKey(HystrixCommandGroupKey.Factory.asKey("antzuhl_zeus"))
    ...
    private class GetZeusFiltersCommand extends AbstractCommand<List<FilterInfo>> {
        private String filterId;

        public GetZeusFiltersCommand(String filterId) {
            this.filterId = filterId;
        }

        @Override
        protected List<FilterInfo> run() throws Exception {
            return delegation.getZeusFilters(filterId);
        }
    }
