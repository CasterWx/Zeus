...
import com.antzuhl.zeus.core.util.JSONUtils;
...
public class FilterScriptManagerServlet extends HttpServlet {
    ...
    public static final DynamicBooleanProperty adminEnabled =
            DynamicPropertyFactory.getInstance().getBooleanProperty(Constants.ZEUS_FILTER_ADMIN_ENABLED, true);
    ...
    private IZeusFilterDao scriptDAO;
    ...
    public FilterScriptManagerServlet() {
        this(ZeusFilterDaoFactory.getZeusFilterDao());
     ...
     * @param scriptDAO
     */
    private FilterScriptManagerServlet(IZeusFilterDao scriptDAO) {
        super();
    ...
    protected void doPut(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (! adminEnabled.get()) {
            response.sendError(HttpServletResponse.SC_FORBIDDEN, "Filter admin is disabled. See the zeus.filters.admin.enabled FastProperty.");
            return;
        }
    ...
    private void handleListAction(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (filter_id == null) {
            ...
            if (Boolean.parseBoolean(request.getParameter("active"))) {
                ...
                // get list of all scripts for this endpoint
                scripts = scriptDAO.getZeusFilters(filter_id);
            }
            if (scripts.size() == 0) {
