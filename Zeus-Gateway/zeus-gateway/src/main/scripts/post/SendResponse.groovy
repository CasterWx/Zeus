...
import com.antzuhl.zeus.core.common.*
import com.antzuhl.zeus.core.context.RequestContext
import com.antzuhl.zeus.core.filters.ZeusFilter

public class SendResponse extends ZeusFilter {
	private static final DynamicIntProperty BUFFER_SIZE = DynamicPropertyFactory.getInstance().getIntProperty(Constants.ZEUS_INITIAL_STREAM_BUFFER_SIZE, 1024);

    private static final DynamicBooleanProperty CONTENT_LENGTH = DynamicPropertyFactory.getInstance().getBooleanProperty(Constants.ZEUS_SET_CONTENT_LENGTH, false);
	...
	public Object run() {
		...
		try{
			...
			final String requestEncoding = context.getRequest().getHeader(ZeusHeaders.ACCEPT_ENCODING)
			if (requestEncoding != null && requestEncoding.contains("gzip")) {
				isGZipRequested = true;
			}
			if (isGZipRequested) {
				servletResponse.setHeader(ZeusHeaders.CONTENT_ENCODING, "gzip")
			} else {
				try {
	...
	private void addResponseHeaders() {
		RequestContext context = RequestContext.getCurrentContext();
		HttpServletResponse servletResponse = context.getResponse();
		List<Pair<String, String>> zeuslResponseHeaders = context.getZeusResponseHeaders();

		zeuslResponseHeaders?.each { Pair<String, String> it ->
