...
import com.antzuhl.zeus.core.hystrix.ZeusRequestCommandForSemaphoreIsolation
import com.antzuhl.zeus.core.hystrix.ZeusRequestCommandForThreadIsolation
import com.antzuhl.zeus.core.util.Debug
import com.antzuhl.zeus.core.util.HTTPRequestUtil

public class ExecuteRoute extends ZeusFilter{
	private static Logger logger = LoggerFactory.getLogger(ExecuteRoute.class);
	private static final DynamicIntProperty MAX_CONNECTIONS = DynamicPropertyFactory.getInstance().getIntProperty(Constants.ZEUS_CLIENT_MAX_CONNECTIONS, 500)
	private static final DynamicIntProperty MAX_CONNECTIONS_PER_ROUTE = DynamicPropertyFactory.getInstance().getIntProperty(Constants.ZEUS_CLIENT_ROUTE_MAX_CONNECTIONS, 20)
	private static final DynamicBooleanProperty DEBUG_ROUTE = DynamicPropertyFactory.getInstance().getBooleanProperty("zeus.debug.thread.route", false);
	...
	public boolean shouldFilter() {
		...
		return ctx.getRouteUrl() != null && ctx.sendZeusResponse();
	}
	...
	public Object run() {
		...
		try{
			...
			Header[] headers = buildZeusRequestHeaders(request)
			...
			throw new ZeusException(errorMsg,500,"TargetUrl: ${targetUrl}\nCause: ${errorMsg}")
		}finally{
			if(DEBUG_ROUTE.get()){
	...
	def Header[] buildZeusRequestHeaders(HttpServletRequest request) {
		...
		zeusRequestHeaders.entrySet().each {
	...
	def HttpResponse forward(HttpClient httpclient, RequestConfig requestConfig, String verb, String url, Header[] headers, InputStream requestEntity, int contentLength, String groupName, String routName) {
		...
		if (isolationStrategy == null) {
			isolationStrategy = DynamicPropertyFactory.instance.getStringProperty("zeus.isolation.strategy.global", "SEMAPHORE").get();
		}

		long start = System.currentTimeMillis();
		try {
			if("THREAD".equalsIgnoreCase(isolationStrategy)){
				return new ZeusRequestCommandForThreadIsolation(httpclient, httpUriRequest, groupName, routName).execute();
			}else{
				return new ZeusRequestCommandForSemaphoreIsolation(httpclient, httpUriRequest, groupName, routName).execute();
			}
		} finally {
	...
	boolean isValidZeusRequestHeader(String name) {
	...
	private RequestConfig buildRequestConfig(String routName, String groupName) {
		...
		if (connectTimeout == 0) {
			connectTimeout = DynamicPropertyFactory.instance.getIntProperty("zeus.connect.timeout.global", 2000).get();
		}
		...
		if (socketTimeout == 0) {
			socketTimeout = DynamicPropertyFactory.instance.getIntProperty("zeus.socket.timeout.global", 10000).get();
		}
		...
		if (requestConnectionTimeout == 0) {
			requestConnectionTimeout = DynamicPropertyFactory.instance.getIntProperty("zeus.request.connection.timeout.global", 10).get();
		}
