...
import com.antzuhl.zeus.core.hystrix.ZeusCommandHelper;
import com.antzuhl.zeus.core.hystrix.RestClient;
import com.antzuhl.zeus.core.hystrix.RestClientFactory;
import com.antzuhl.zeus.core.hystrix.RibbonRequestCommandForSemaphoreIsolation;
import com.antzuhl.zeus.core.hystrix.RibbonRequestCommandForThreadIsolation;
import com.antzuhl.zeus.core.util.Debug;
import com.antzuhl.zeus.core.util.HTTPRequestUtil;

public class ExecuteRibbon extends ZeusFilter {
	...
	public boolean shouldFilter() {
		...
		return ctx.getRoute() != null && ctx.sendZeusResponse();
	}
	...
	public Object run() throws ZeusException {
		...
		try {
			...
			Collection<Header> headers = buildZeusRequestHeaders(RequestContext.getCurrentContext().getRequest());
			...
			throw new ZeusException(errorMsg, 500, ",serviceName: ${serviceName}\nCause: ${errorMsg}\nUri:${uri}");
		} finally {
			tran.complete();
		}
		return null;
	...
	public HttpResponse forward(AbstractLoadBalancerAwareClient client,IClientConfig clientConfig, String verb, String uri, String serviceName,
		...
		if (isolationStrategy == null) {
			isolationStrategy = DynamicPropertyFactory.getInstance()
					.getStringProperty("zeus.isolation.strategy.global", "SEMAPHORE").get();
		}
	...
	void setResponse(HttpResponse response) throws ClientException, IOException {
		...
		for (String headerName : map.keySet()) {
			...
			if (headerName.equalsIgnoreCase(ZeusHeaders.CONTENT_LENGTH)) {
				ctx.setOriginContentLength(headerValue);
			}
			if (isValidZeusResponseHeader(headerName)) {
				ctx.addZeusResponseHeader(headerName, headerValue);
			}
			if (headerName.equalsIgnoreCase(ZeusHeaders.CONTENT_ENCODING)) {
	...
	public InputStream debug(String verb, String url, Collection<Header> headers, InputStream requestEntity,
		...
		if (Debug.debugRequest()) {
			RequestContext.getCurrentContext().addZeusResponseHeader("x-target-url", url);
			Debug.addRequestDebug("ZEUS:: url=${url}");
			headers.each {
				Debug.addRequestDebug("ZEUS::> ${it.name}  ${it.value}")
			}
	...
	boolean isValidZeusResponseHeader(String name) {
	...
	InputStream debugRequestEntity(InputStream inputStream) throws IOException {
		...
		Debug.addRequestDebug("ZEUS::> ${entity}")
		return new ByteArrayInputStream(entityBytes);
	}


	private Collection<Header> buildZeusRequestHeaders(HttpServletRequest request) {
		...
		while (headerNames.hasMoreElements()) {
			...
			if (isValidZeusRequestHeader(name)) {
				headersMap.put(name, new BasicHeader(name, value));
			}
		}

		Map<String, String> zeusRequestHeaders = RequestContext.getCurrentContext().getZeusRequestHeaders();
		for (String key : zeusRequestHeaders.keySet()) {
			String name = key.toLowerCase();
			String value = zeusRequestHeaders.get(key);
			headersMap.put(name, new BasicHeader(name, value));
		}
	...
	public boolean isValidZeusRequestHeader(String name) {
	...
	private IClientConfig buildRequestConfig(String serviceName,String routeName,String routeGroup) {
		...
		clientConfig.setProperty(CommonClientConfigKey.NIWSServerListClassName, "com.antzuhl.zeus.core.hystrix.DiscoveryServerList");
		clientConfig.setProperty(CommonClientConfigKey.ClientClassName, "com.antzuhl.zeus.core.hystrix.RestClient");
		...
		clientConfig.setProperty(CommonClientConfigKey.NFLoadBalancerRuleClassName, ZeusCommandHelper.getRibbonLoadBalanceRule(routeGroup, routeName));
		clientConfig.setProperty(CommonClientConfigKey.MaxHttpConnectionsPerHost, ZeusCommandHelper.getRibbonMaxHttpConnectionsPerHost(serviceName));
		clientConfig.setProperty(CommonClientConfigKey.MaxTotalHttpConnections, ZeusCommandHelper.getRibbonMaxTotalHttpConnections(serviceName));
		clientConfig.setProperty(CommonClientConfigKey.MaxAutoRetries, ZeusCommandHelper.getRibbonMaxAutoRetries(serviceName));
		clientConfig.setProperty(CommonClientConfigKey.MaxAutoRetriesNextServer, ZeusCommandHelper.getRibbonMaxAutoRetriesNextServer(serviceName));
		clientConfig.setProperty(CommonClientConfigKey.ConnectTimeout, ZeusCommandHelper.getRibbonConnectTimeout(routeGroup,routeName));
		clientConfig.setProperty(CommonClientConfigKey.ReadTimeout, ZeusCommandHelper.getRibbonReadTimeout(routeGroup,routeName));
